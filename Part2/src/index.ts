...
const encrypt = async (
  ...
  const iv = mimc7.getIV(mimc7.multiHash(plaintext, BigInt(0)));
  console.log('iv: ---->', iv);
  const ciphertext: Ciphertext = {
    iv,
    data: plaintext.map((e: bigint, i: number): bigint => {
      const hash = mimc7.F.toObject(mimc7.hash(iv + BigInt(i), sharedKey));
...
const decrypt = async (
  ...
  const plaintext: Plaintext = ciphertext.data.map(
    (e: bigint, i: number): bigint => {
      const hash = mimc7.F.toObject(mimc7.hash(iv + BigInt(i), sharedKey)); // hash the shared key with the IV
      return BigInt(e) - hash;
    },
  );
